@include('front.layouts.header')
@include('front.layouts.nav')


    <!-- <div class="page-load-outer">
        <div class="page-load">

        </div>
    </div> -->





    <div class="festival">

        <div class="banner">
            <!--style-->
        </div>

        <div class="content">
            <div class="row">
                <div class="col-md-3">
                    <div class="festival-image-1">
                        <img src="{{asset('front/')}}/images/selale.jpg">
                    </div>
                </div>
                <div class="col-md-9">

                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="top-title">
                             <!-- <h3>Sındırgı'da hikayesi olam Festival.. Uluslararaso Sındırgı Yağcıbedir Halı, Kültür, ve Sanat Günleri</h3> -->
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="title">
                            <h3>Sındırgı'da hikayesi olan Festival..<br> Uluslararası Sındırgı Yağcıbedir Halı, Kültür, ve Sanat Günleri</h3>
                        </div>
                        <div class="text">
                            <p>Türkiye'de yerel tezgahlarda dokunan tek halı olan dünyaca ünlü Yağcıbedir halısını yaşatma ve el sanatlarımızı yok olmasını engellemek amacıyla
                                her yıl Ağustos sonu Eylül başı <span>uluslararası</span> Sındırgı Yağcıbedir halı kültür ve sanat günleri gerçekleştirilmektedir. Bir festivalden öte bir hikayesi olan, kültür ve sanatın buluştuğu yöresel bir şölen gerçekleştirilmektedir. <br><br>

                                <b><p>Yapılan Etlinlikler</p></b>
                                <ul>
                                    <li> Yağcıbedir Yağlı Pehlivan Güreşleri</li>
                                    <li> Yerli ve Yabancı Konukların Folklor Gösterileri</li>
                                    <li> En Güzel Yağcıbedir Halısı Yarışması</li>
                                    <li> Yöresel Atlı Cirit Müsabakası</li>
                                    <li> Tek Kurşun Atış Yarışması</li>
                                    <li> Yöresel Yemek Yarışması</li>
                                    <li> Sındırgı Bisiklet Maratonu</li>
                                    <li> Kuva-i Milliye Korteji</li>
                                    <li> Konserler..</li>
                                </ul>

                            </p>
                        </div>
                    </div>
                </div>
            </div>

            <div class="row">
                <div class="col-md-9">

                </div>
                <div class="col-md-3">
                    <div class="festival-image-2">
                        <img src="{{asset('front/')}}/images/festival-10.jpg">
                    </div>
                </div>
            </div>


            <div class="festival-photo-gallery">
                <div class="container">
                    <div class="row">
                        <div class="col-md-3">
                            <div class="image">
                                <a href="{{asset('front/')}}/images/festival-2.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                    <img src="{{asset('front/')}}/images/festival-2.jpg" />
                                </a>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="image-2">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-3.jpg" data-fancybox="gallery"
                                        data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-3.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="image-3">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-1.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-4.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-2">
                            <div class="image-4">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-5.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-5.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-5">
                            <div class="image-5">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-6.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-6.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-5">
                            <div class="image-6">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-7.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-7.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-3">
                            <div class="image">
                                <a href="{{asset('front/')}}/images/festival-8.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                    <img src="{{asset('front/')}}/images/festival-8.jpg" />
                                </a>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="image-2">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-9.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-9.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="image-3">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-10.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-10.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-2">
                            <div class="image-4">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-11.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-11.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-5">
                            <div class="image-5">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-12.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-12.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-5">
                            <div class="image-6">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-13.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-13.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-3">
                            <div class="image">
                                <a href="{{asset('front/')}}/images/festival-14.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                    <img src="{{asset('front/')}}/images/festival-14.jpg" />
                                </a>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="image-2">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-15.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-15.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="image-3">
                                <div class="image">
                                    <a href="{{asset('front/')}}/images/festival-16.jpg" data-fancybox="gallery" data-caption="Caption #1">
                                        <img src="{{asset('front/')}}/images/festival-16.jpg" />
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>


    @include('front.layouts.footer')
